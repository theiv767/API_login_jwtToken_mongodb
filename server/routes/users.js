...
router.post('/auth/login/', async (req, res, next) => {
    ...
    try {
        ...
        console.log(e)
        res.status(201).json({"error": "Falha ao efetuar login !"})
    }
})
...
router.get('/', async (req, res) => {
    try {
        ...
        console.log(error)
        res.status(500).json({ "error": "Erro ao tentar exibir usuários" })
    }
})


// rota privada. Precisa passar pela checagem do token
// ====== GET especifico ===================================================================================
router.get('/:id', checkToken, async (req, res) => {
    const id = req.params.id

    try {
        // puxa todos os dados do unuário, menos a senha
...
//PUT
//=========== ALTERAR UM ATRIBUTO ESPECIFICO ====================================================
